...
if (options.mode === 'server') {
    ...
    const server = new Server_1.Server(myKey, 8889);
    const xrpawn = new Blockchain_1.Blockchain(myKey, [
    ...
    for (var i = 0; i < 100; i++) {
        const tx = new Transaction_1.Transaction(myAddress, myAddress2, 10);
        tx.sign(myKey);
    ...
    server.getRouter().subscribeTopic('chain', (publicationId, session, chain) => {
        console.log('chain :', chain);
    });
    server.getRouter().registerRPC('check', (id, session, lastBlockHash) => {
        return xrpawn.getLatestBlock().hash === lastBlockHash;
    });
    server.getRouter().registerRPC('sync', (id, session, index, blocks) => {
        ...
        for (let i = index; i < xrpawn.chain.length && i < maxLength; i++) {
            ...
            for (const blockData of blocks) {
                const block = Block_1.Block.fromData(blockData);
    ...
    const masterKey = ec.keyFromPrivate('049589f0c3a1b31e7d55379bf3ea66de62bed7dad6c247cc8ecf30bed939e910');
    const client = new Client_1.Client(masterKey, '127.0.0.1', 8889);
    const xrpawn = new Blockchain_1.Blockchain(myKey, [
        (new Transaction_1.Transaction(masterToken, myAddress, 30000)),
        (new Transaction_1.Transaction(masterToken, myAddress2, 2453)),
        (new Transaction_1.Transaction(masterToken, myAddress3, 1000)),
    ]);
    function syncChain(session, syncIndex = 0) {
        return new Promise((resolve, reject) => {
            console.log('Sync : ', syncIndex, '...');
            session
                .call('sync', [syncIndex, xrpawn.chain.slice(syncIndex, 2)])
                .then((data) => {
                console.log('Sync : ', data.index, data.done, data.blocks.length || 'no sync', 'OK');
                data.blocks = data.blocks.map((block) => Block_1.Block.fromData(block));
